...
`proxy/syslog_worker.py:start_syslog_thread()` is called at app startup. It spawns a daemon thread (`syslog-forwarder`) that wakes every 10 s and, when `syslog_enabled=1` is set in `AppConfig`, reads new rows from `modsec_raw_log` (WAF events) and `audit_log` (admin/system actions) using incremental cursors (`syslog_cursor_modsec_id`, `syslog_cursor_audit_id`) stored in `AppConfig`. Each row is serialized as an **RFC5424** message (body = JSON) and sent to the configured syslog server via UDP or TCP (octet-counting RFC6587). Cursor advances only after a successful batch — at-least-once semantics. On first activation the cursor bootstraps to `MAX(id)` so no historical data is replayed. PostgreSQL advisory lock **815005** (SOC=815001, backup=815002, port_monitor=815003, rawlog=815004) prevents duplicate delivery under gunicorn multi-worker. Logic lives in `proxy/syslog_forwarder.py` (pure functions: `build_rfc5424`, `SyslogSender`, severity mapping) and `proxy/syslog_worker.py` (worker loop, cursors, `AppConfig` telemetry). No Docker dependency; no new DB tables. nginx access/error logs are not forwarded (they remain in Loki/Grafana via Alloy).
